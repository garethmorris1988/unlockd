...
// Bundled local sounds — generated by scripts/generate-sounds.js
const SOUND_BELL     = require('../../assets/sounds/bell.wav')
const SOUND_COMPLETE = require('../../assets/sounds/complete.wav')
...
export default function MeditationScreen() {
  ...
  async function playTone(type: 'minute' | 'complete') {
    try {
      const source = type === 'complete' ? SOUND_COMPLETE : SOUND_BELL
      const { sound } = await Audio.Sound.createAsync(source, { shouldPlay: true })
      sound.setOnPlaybackStatusUpdate(status => {
        if (status.isLoaded && status.didJustFinish) sound.unloadAsync()
      })
    } catch (e) {
      // Silently ignore audio errors — meditation continues without sound
    }
  }
      ...
      <View style={{ flex: 1, paddingHorizontal: 28 }}>
        ...
        {isComplete && (
          <Text style={{ fontSize: 13, color: '#999', textAlign: 'center', lineHeight: 20 }}>
            {firstName ? `Well done, ${firstName}. Your mind is ready.` : 'Well done. Your mind is ready.'}
          </Text>
        )}
